...
powershell -NoProfile -ExecutionPolicy Bypass -Command "$root=(Resolve-Path -LiteralPath '.').Path; $frontend=Join-Path $root 'frontend'; $targets=@(Get-CimInstance Win32_Process -ErrorAction SilentlyContinue | Where-Object { (($_.Name -eq 'python.exe') -or ($_.Name -eq 'pythonw.exe') -or ($_.Name -eq 'node.exe') -or ($_.Name -eq 'cmd.exe')) -and (($_.CommandLine -like \"*$root*app.py*\") -or ($_.CommandLine -like \"*$root*api_server*\") -or ($_.CommandLine -like \"*$frontend*\" -and $_.CommandLine -like '*vite*')) }); if ($targets.Count -gt 0) { $targets | ForEach-Object { Stop-Process -Id $_.ProcessId -Force -ErrorAction SilentlyContinue } }; Remove-Item -LiteralPath (Join-Path $root '%PID_FILE%') -Force -ErrorAction SilentlyContinue; Remove-Item -LiteralPath (Join-Path $root '%VITE_PID_FILE%') -Force -ErrorAction SilentlyContinue"
