...
func (p *processor) watchDir(inputDir string) error {
	...
	go func() {
		defer func() {
			if r := recover(); r != nil {
				p.logger.Error("watcher goroutine panic", zap.Any("error", r))
			}
		}()
		...
		for {
			select {
			case <-ctx.Done():
				p.logger.Debug("watcher exiting gracefully")
				return
...
func (p *processor) processDir(inputDir string) error {
	...
	// Recurse into subdirectories first (sequential, avoids goroutine explosion).
	var lastError error
	for _, item := range dirs {
		if err := p.processDir(filepath.Join(inputDir, item.Name())); err != nil {
			lastError = err
		}
	}
	...
	for _, item := range files {
		...
		go func(fp string, name string) {
			defer wg.Done()
			defer func() { <-sem }() // release slot
			if err := p.processFile(fp); err != nil {
				mu.Lock()
				lastError = err
				mu.Unlock()
				p.logger.Error("conversion failed", zap.String("source", name), zap.Error(err))
			}
		}(filePath, item.Name())
	}
	wg.Wait()
	...
	if p.updateMetadata && params.Meta != nil {
		if coverGetter, ok := dec.(common.CoverImageGetter); ok {
			...
			if cover, err := coverGetter.GetCoverImage(ctx); err != nil {
				...
				logger.Warn("sniff cover image type failed",
					zap.Int("cover_len", len(cover)),
					zap.Binary("cover_header", cover[:min(8, len(cover))]),
				)
			} else {
