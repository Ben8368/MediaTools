...
from text_modifier import modify_text_layer, AdjustParams, ModifyResult
...
def scan_document_for_ticket(ps, doc, source_psd: str) -> list[TicketScanRow]:
...
def _expand_ticket_targets(languages: list[str], output_names: list[str]) -> list[tuple[str, str]]:
    ...
    return list(zip(languages, output_names))
...
def read_ticket_csv(filepath: str) -> list[TicketRow]:
    ...
    with open(filepath, "r", encoding="utf-8-sig") as f:
        ...
        missing = set(TICKET_FIELDNAMES) - headers
        if missing:
        ...
        for idx, row in enumerate(reader, start=2):
            try:
                ...
                raise ValueError(f"工单第 {idx} 行解析错误: {e}")
    return rows
...
def execute_ticket(ps, psd_path: str, ticket_rows: list[TicketRow], output_dir: str, params: AdjustParams, font_metrics_path: str = None) -> dict[str, list[ModifyResult]]:
    ...
    if font_metrics_path and os.path.exists(font_metrics_path):
        ...
        print(f"  [font_metrics] 未找到缓存，将使用 Bounds 自适应法（fallback）")
    ...
    for output_name, rows in grouped.items():
        ...
        try:
            ...
            scan_map = {row.layer_id: row for row in scanned}
            results: list[ModifyResult] = []
            for ticket_row in rows:
                if not _should_apply_ticket_row(ticket_row):
                    continue
                target = scan_map.get(ticket_row.layer_id)
                if target is None:
                    results.append(ModifyResult(
