...
function __freshManifest(mainFile) {
    return {
        version: 1,
        project: {
...
function __loadManifest(mainFile) {
    ...
    if (!f.exists) {
        return __freshManifest(mainFile);
    }
    if (!f.open('r')) {
    ...
    // Empty file = corrupt manifest (likely truncated by interrupted write)
    if (!raw || !raw.length) {
        var msg = 'CHECKPOINT_DIAG: manifest is empty (0 bytes), path=' + f.fsName
            + ', fileSize=' + f.length + ', exists=' + f.exists;
        try { $.writeln(msg); } catch (_) {}
        try { if (typeof Logger !== 'undefined') Logger.error(msg); } catch (_) {}

        // Check for backup
        var bak = new File(f.fsName + '.bak');
        if (bak.exists && bak.length > 0) {
            try {
                bak.open('r');
                var bakRaw = bak.read();
                bak.close();
                var restored = JSON.parse(bakRaw);
                var rMsg = 'CHECKPOINT_DIAG: restored manifest from .bak (' + bakRaw.length + ' bytes)';
                try { $.writeln(rMsg); } catch (_) {}
                try { if (typeof Logger !== 'undefined') Logger.info(rMsg); } catch (_) {}
                return restored;
            } catch (bakErr) {
                var bMsg = 'CHECKPOINT_DIAG: .bak also corrupt: ' + bakErr.toString();
                try { $.writeln(bMsg); } catch (_) {}
            }
        }
        return __freshManifest(mainFile);
    }
    ...
    try {
        ...
        var errMsg = 'CHECKPOINT_DIAG: manifest parse failed, path=' + f.fsName
            + ', rawLength=' + raw.length
            + ', first100=' + raw.substring(0, 100)
            + ', error=' + e.toString();
        try { $.writeln(errMsg); } catch (_) {}
        try { if (typeof Logger !== 'undefined') Logger.error(errMsg); } catch (_) {}
...
function __saveManifest(mainFile, manifest) {
    ...
    if (!content || !content.length) {
        throw new Error('CHECKPOINT_DIAG: JSON.stringify returned empty for manifest');
    }

    var target = __manifestFile(mainFile);

    // Write to temp file first, then rename — prevents 0-byte manifest on crash
    var tmp = new File(target.fsName + '.tmp');
    if (!tmp.open('w')) {
        throw new Error('Failed to open manifest temp file: ' + tmp.error);
    }
    tmp.write(content);
    tmp.close();
    ...
    if (!tmp.exists || tmp.length === 0) {
        throw new Error('CHECKPOINT_DIAG: temp manifest is empty after write, path=' + tmp.fsName);
    }

    // Backup current manifest before overwriting
    if (target.exists && target.length > 0) {
        var bak = new File(target.fsName + '.bak');
        target.copy(bak);
    }

    // Atomic-ish rename: copy temp over target, then remove temp
    var ok = tmp.copy(target);
    if (!ok) {
        throw new Error('Failed to copy temp manifest to target: ' + tmp.error);
    }
    tmp.remove();
}
...
function atom_createCheckpoint(stepMetaJson) {
    ...
    try {
        ...
        // Only flush to disk when there are unsaved in-memory changes.
        // Skipping the save when dirty===false avoids the expensive full
        // project serialisation (30-60s on large projects).
